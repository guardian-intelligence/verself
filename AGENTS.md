...
se River OSS as the worker/queue substrate, keep the execution state machine explicit in Postgres, and delete/rewrite the current jobs code instead of refactoring it in place. Do not use River Pro features as a foundational dependency. If we need global concurrency beyond a single process, model it in
