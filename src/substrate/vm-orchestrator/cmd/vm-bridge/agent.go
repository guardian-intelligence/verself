...
func (s *agentSession) sealFilesystem(name string) error {
	...
	syscall.Sync()
	if err := unmountFilesystem(fs); err != nil {
		return err
	}
...
func unmountFilesystem(fs vmproto.FilesystemMount) error {
	for i := len(fs.BindPaths) - 1; i >= 0; i-- {
		bindPath := filepath.Clean(fs.BindPaths[i])
		if err := syscall.Unmount(bindPath, 0); err != nil && err != syscall.EINVAL && err != syscall.ENOENT {
			return fmt.Errorf("unmount filesystem %s bind %s: %w", fs.Name, bindPath, err)
		}
	}
	if err := syscall.Unmount(fs.MountPath, 0); err != nil && err != syscall.EINVAL && err != syscall.ENOENT {
		return fmt.Errorf("unmount filesystem %s at %s: %w", fs.Name, fs.MountPath, err)
	}
	return nil
}
...
func (s *agentSession) mountFilesystem(fs vmproto.FilesystemMount) vmproto.FilesystemMountResult {
	...
	if !fs.ReadOnly {
		...
		for _, bindPath := range fs.BindPaths {
			if err := bindFilesystemPath(fs, bindPath); err != nil {
				_ = unmountFilesystem(fs)
				result.Error = err.Error()
				return result
			}
		}
	...
	_, _ = fmt.Fprintf(os.Stdout, "%s mounted composed filesystem name=%s device=%s path=%s bind_count=%d read_only=%t\n", logPrefix, fs.Name, fs.DevicePath, fs.MountPath, len(fs.BindPaths), fs.ReadOnly)
	s.filesystems = append(s.filesystems, fs)
	result.Mounted = true
	return result
}

func bindFilesystemPath(fs vmproto.FilesystemMount, target string) error {
	target = filepath.Clean(strings.TrimSpace(target))
	if err := validateMountPath("bind path", fs.Name, target); err != nil {
		return err
	}
	if stat, err := os.Stat(target); err == nil {
		if !stat.IsDir() {
			return fmt.Errorf("bind target %s for filesystem %s is not a directory", target, fs.Name)
		}
	...
	source := filepath.Join(fs.MountPath, "paths", strings.TrimPrefix(target, string(os.PathSeparator)))
	if err := os.MkdirAll(source, 0o755); err != nil {
		return fmt.Errorf("mkdir bind source %s: %w", source, err)
	}
	if err := os.Chown(source, runnerUID, runnerGID); err != nil {
		return fmt.Errorf("chown bind source %s: %w", source, err)
	}
	if err := syscall.Mount(source, target, "", syscall.MS_BIND, ""); err != nil {
		return fmt.Errorf("bind mount filesystem %s source %s on %s: %w", fs.Name, source, target, err)
	}
	remountFlags := uintptr(syscall.MS_BIND | syscall.MS_REMOUNT | syscall.MS_NOATIME | syscall.MS_NODEV | syscall.MS_NOSUID)
...
func validateFilesystemMount(fs vmproto.FilesystemMount) error {
	...
	if err := validateMountPath("mount path", fs.Name, fs.MountPath); err != nil {
		return err
	}
	seenBindPaths := map[string]struct{}{}
	for _, bindPath := range fs.BindPaths {
		if err := validateMountPath("bind path", fs.Name, bindPath); err != nil {
			return err
		}
		clean := filepath.Clean(bindPath)
		if _, ok := seenBindPaths[clean]; ok {
			return fmt.Errorf("composed filesystem %s bind path %q is duplicated", fs.Name, bindPath)
		}
		seenBindPaths[clean] = struct{}{}
	}
...
func validateMountPath(kind, name, value string) error {
	if strings.TrimSpace(value) == "" || !strings.HasPrefix(value, "/") {
		return fmt.Errorf("composed filesystem %s has invalid %s %q", name, kind, value)
	}
	clean := filepath.Clean(value)
	if clean != value || clean == "/" || strings.HasPrefix(clean, "/proc") || strings.HasPrefix(clean, "/sys") || strings.HasPrefix(clean, "/dev") || strings.HasPrefix(clean, "/run") {
		return fmt.Errorf("composed filesystem %s %s %q is not allowed", name, kind, value)
	}
	return nil
}
