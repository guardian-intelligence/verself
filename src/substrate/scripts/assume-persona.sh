...
# shellcheck source=src/substrate/scripts/lib/verification-context.sh
...
if [[ ${print_env} -eq 0 && -z "${output_path}" ]]; then
  output_path="${VERIFICATION_SMOKE_ARTIFACT_ROOT}/personas/${persona}.env"
fi
...
write_export VERSELF_WEB_URL "https://${VERIFICATION_DOMAIN}"
