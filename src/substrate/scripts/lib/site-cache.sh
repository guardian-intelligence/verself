...
#   VERSELF_RENDER_CACHE_DIR    — absolute path to src/substrate/ansible
#   VERSELF_ANSIBLE_INVENTORY   — absolute path to src/substrate/ansible/inventory
...
site_cache_init() {
  ...
  if [[ ! -d "${repo_root}/src/substrate/ansible" ]]; then
    echo "site-cache.sh: failed to locate repo root from ${script_dir}" >&2
    return 1
  fi

  VERSELF_RENDER_CACHE_DIR="${repo_root}/src/substrate/ansible"
