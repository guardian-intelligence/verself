...
- `scripts/` contains the per-layer runner (`run-layer.sh`), the ledger
  ...
  (`layer-last-applied.sh`), the post-deploy canary
  (`divergence-canary.sh`), and the OTLP wrapper (`ansible-with-otel.sh`).
...
playbook through `scripts/ansible-with-otel.sh` (writes a `succeeded` /
