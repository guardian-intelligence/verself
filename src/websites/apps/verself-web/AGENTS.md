...
Read @src/websites/packages/ui/src/components/ui/page.tsx for visual hierarchy rules.

Lives at `src/websites/apps/verself-web/` within a pnpm workspace managed by Vite+. Shared UI primitives (`cn`, `Skeleton`) come from canonical `@verself/ui` subpath exports (`packages/ui/`).
