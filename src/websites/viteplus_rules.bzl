...
def viteplus_route_tree(name):
    ...
    js_run_binary(
        name = gen_target,
        srcs = [
            "//src/websites:node_modules/@tanstack/router-generator",
            ":sources",
        ],
        outs = [out],
        args = [
        ...
        chdir = native.package_name(),
        tool = "//src/websites/scripts:generate_route_tree",
...
def viteplus_openapi_clients(name, specs, openapi_ts_bin, plugin_packages = None):
    ...
    for sdk_dir, spec_label in specs:
        ...
        js_run_binary(
            ...
            tool = "//src/websites/scripts:openapi_postprocess",
