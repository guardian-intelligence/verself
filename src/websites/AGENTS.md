...
- The Vite+ toolchain is invoked only as `aspect vp -- <args>` (lockfile-pinned `node_modules/vite-plus` under the server_tools Node; no global `vp`, no `~/.vite-plus`). See `src/websites/README.md`.
