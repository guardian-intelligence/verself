...
cd to `src/viteplus-monorepo` and run commands such as `vp install --frozen-lockfile`,
`vp check .`, and `vp test run`. For app development, cd to the app package and
run its local command, for example `cd src/viteplus-monorepo/apps/verself-web && vp dev`.
...
```bash
cd src/viteplus-monorepo/apps/verself-web
vp dev
```
