...
const AMBIENT_STRENGTH_SCALE = 1.8;
  ...
  return {
    collect(input: AmbientWaveCollectInput): ReadonlyArray<WaveImpulse> {
      ...
      nextWaveSeconds =
        input.nowSeconds +
        randomRange(random, WAVE_INTERVAL_SECONDS.min, WAVE_INTERVAL_SECONDS.max);
      const wave = createAmbientWave(random, input, recentWaves);
      if (wave === undefined) {
        return [];
      }

      rememberWave(recentWaves, wave);
...
function createAmbientWave(
  ...
  recentWaves: readonly RecentAmbientWave[],
  ...
  const sweep = chooseHaloSweep(random, input, recentWaves, radius, focusPoint);
  ...
  return {
    ...
    origin: sweep.origin,
    radius,
    strength: baseStrength * randomRange(random, 0.84, 1.02),
  };
}
...
function chooseHaloSweep(
  ...
  const waveScale = viewportWaveScale(input.viewport);
  const minOriginDistance = 0.44 * interpolate(0.78, 1, waveScale);
...
function recentWavePenalty(
  candidate: { readonly direction: Vec2; readonly origin: Vec2 },
  recentWaves: readonly RecentAmbientWave[],
  minOriginDistance: number,
): number {
  let penalty = 1;
  for (const recentWave of recentWaves) {
    const distance = Math.hypot(
      candidate.origin.x - recentWave.origin.x,
      candidate.origin.y - recentWave.origin.y,
    );
    ...
    const alignment = Math.max(
      0,
      candidate.direction.x * recentWave.direction.x +
        candidate.direction.y * recentWave.direction.y,
    );
    const repeatedPhaseDistance = minOriginDistance * 1.55;
    if (alignment > 0.72 && distance < repeatedPhaseDistance) {
      return 0;
    }

    const distancePenalty = smoothstep(minOriginDistance, minOriginDistance * 2.1, distance);
    const phasePenalty =
      1 -
      smoothstep(0.66, 0.94, alignment) *
        (1 - smoothstep(repeatedPhaseDistance, repeatedPhaseDistance * 1.7, distance)) *
        0.52;
    penalty *= distancePenalty * phasePenalty;
  }
  return penalty;
}

function rememberWave(recentWaves: RecentAmbientWave[], wave: AmbientWave) {
  recentWaves.push({
    direction: wave.direction,
    origin: wave.origin,
  });
  while (recentWaves.length > RECENT_WAVE_LIMIT) {
    recentWaves.shift();
  }
}
