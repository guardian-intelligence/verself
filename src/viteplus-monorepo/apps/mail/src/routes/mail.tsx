      ...
      <div className="flex items-center justify-center h-full">
        <div className="text-center space-y-3 max-w-sm px-4">
          <div className="mx-auto w-12 h-12 rounded-full bg-muted flex items-center justify-center">
            <svg viewBox="0 0 24 24" className="w-6 h-6 text-muted-foreground" fill="none" stroke="currentColor" strokeWidth="1.5">
              <path d="M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728A9 9 0 015.636 5.636m12.728 12.728L5.636 5.636" />
            </svg>
          </div>
          <p className="text-lg font-medium">No mailbox linked</p>
          <p className="text-sm text-muted-foreground">
            Your account is authenticated but not bound to a mailbox.
            Ask your administrator to create a mailbox binding for your identity.
          </p>
        </div>
      </div>
    );
...
function MailboxLink({ mailbox, unread }: { mailbox: ElectricMailbox; unread: number }) {
  ...
  return (
    ...
    >
      ...
      {unread > 0 && (
        <span className={`text-xs tabular-nums ${isInbox ? "font-semibold text-sidebar-active-foreground" : "text-muted-foreground"}`}>
          {unread}
        </span>
      )}
    </Link>
