...
function RepoDetailPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {rescanMutation.error ? <ErrorCallout error={rescanMutation.error} title="Rescan failed" /> : null}
      {refreshMutation.error ? <ErrorCallout error={refreshMutation.error} title="Refresh failed" /> : null}
      {runMutation.error ? <ErrorCallout error={runMutation.error} title="Execution launch failed" /> : null}
      {repo.last_error ? (
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {generations.length === 0 ? (
                <TableEmptyRow colSpan={6}>No generations yet.</TableEmptyRow>
              ) : (
