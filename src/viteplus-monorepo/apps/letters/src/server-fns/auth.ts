...
async function getLettersAuthConfig() {
...
export const lettersAuthMiddleware = createMiddleware({ type: "function" }).server(
  async ({ next }) => {
    ...
    const auth = await getAuthSession(await getLettersAuthConfig());
    if (!auth) {
...
export const getClientAuthSnapshot = createServerFn({ method: "GET" }).handler(async () => {
  ...
  return getClientAuthSnapshot(getLettersAuthConfig);
});
