...
export const Route = createFileRoute("/letters/$slug")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.letter.title ?? "Letters"} — Guardian Intelligence` },
      { name: "description", content: loaderData?.letter.summary ?? "" },
    ],
  }),
});
...
function LetterPost() {
  ...
  return (
    <PageShell kicker={`${letter.publishedAt} · ${letter.kicker}`} heading={letter.title}>
