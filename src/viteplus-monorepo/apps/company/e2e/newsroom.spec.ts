...
// Structural test for the /newsroom index. Asserts the Ramp-style layout lands
// end-to-end: the one Flare giant bulletin banner and the article metadata
// strip below. Archive grid, tab strip, pagination, and subscribe band are
// retired until we have a second bulletin and a newsletter service — the rule
// is one Flare giant bulletin per page. Each interaction fires a browser span
// (newsroom.index.view / bulletin_click); deployment verification asserts those
// land in ClickHouse.
//
...
test.describe("/newsroom — index", () => {
  test("renders Flare giant bulletin and article metadata", async ({ page }) => {
    await page.goto("/newsroom");

    await expect(page.locator("main h1")).toHaveCount(0);

    // 1. Exactly one Flare giant bulletin — the rule for Newsroom is one
    ...
    // 2. The Flare bulletin paints plain Flare (#CCFF00) — no pattern, no
    ...
    // 3. Article metadata strip renders under the bulletin and carries the
    // deck copy.
    ...
    // 4. The retired surfaces are gone — no archive grid, no tab strip,
