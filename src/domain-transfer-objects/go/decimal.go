...
func ParseUint64(value string) (uint64, error) {
	if !isUnsignedDecimal(value) {
		return 0, fmt.Errorf("dto: invalid uint64 decimal %q", value)
	}
	parsed, err := strconv.ParseUint(value, 10, 64)
	if err != nil {
		return 0, fmt.Errorf("dto: invalid uint64 decimal %q: %w", value, err)
	}
	return parsed, nil
}
...
func ParseInt64(value string) (int64, error) {
	if !isSignedDecimal(value) {
		return 0, fmt.Errorf("dto: invalid int64 decimal %q", value)
	}
	parsed, err := strconv.ParseInt(value, 10, 64)
	if err != nil {
		return 0, fmt.Errorf("dto: invalid int64 decimal %q: %w", value, err)
	}
	return parsed, nil
}
...
func (d *DecimalUint64) UnmarshalJSON(data []byte) error {
	if bytes.Equal(data, []byte("null")) {
		return fmt.Errorf("dto: decimal uint64 cannot be null")
	}
	var value string
	// Reject JSON numbers; JS clients cannot represent all uint64 values safely.
	if err := json.Unmarshal(data, &value); err != nil {
		return fmt.Errorf("dto: decimal uint64 must be a quoted decimal string: %w", err)
	}
...
func (d *DecimalInt64) UnmarshalJSON(data []byte) error {
	if bytes.Equal(data, []byte("null")) {
		return fmt.Errorf("dto: decimal int64 cannot be null")
	}
	var value string
	// Reject JSON numbers; JS clients cannot represent all int64 values safely.
	if err := json.Unmarshal(data, &value); err != nil {
		return fmt.Errorf("dto: decimal int64 must be a quoted decimal string: %w", err)
	}
