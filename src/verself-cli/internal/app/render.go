...
func writeRenderedFiles(store *Store, company CompanyRecord, repoRoot string, force bool) error {
	siteVarsPath := filepath.Join(repoRoot, "src", "bootstrap", "sites", company.Site, "vars.yml")
	...
	files := map[string][]byte{
		...
		filepath.Join(repoRoot, "src", "bootstrap", "sites", company.Site, "provisioning.tfvars.json.template"): []byte(renderProvisioningTemplate(company)),
