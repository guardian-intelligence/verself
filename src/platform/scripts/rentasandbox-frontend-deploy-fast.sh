#!/usr/bin/env bash
# Fast path for shipping rent-a-sandbox UI changes: build locally, rsync
...
#   - src/viteplus-monorepo/apps/rent-a-sandbox/src/**
#   - src/viteplus-monorepo/packages/**  (UI + env + auth-web)
#
# NOT safe (run the Ansible role via `ansible-playbook ... --tags rent_a_sandbox`):
#   - Go API changed (regenerates __generated/**)
#   - rent-a-sandbox.service.j2 or env.j2 templates changed
...
LOCAL_OUTPUT="${MONOREPO_DIR}/apps/rent-a-sandbox/.output/"
REMOTE_OUTPUT="/opt/rent-a-sandbox/apps/rent-a-sandbox/.output/"
SERVICE_HOST="127.0.0.1"
SERVICE_PORT="4244"

step() { printf '\n\033[1;34m==> %s\033[0m\n' "$*"; }

step "Build @forge-metal/rent-a-sandbox (local, dependency-aware via vp)"
...
vp run "@forge-metal/rent-a-sandbox#build"
popd >/dev/null
...
step "Restart rent-a-sandbox"
verification_ssh sudo systemctl restart rent-a-sandbox
...
echo "rent-a-sandbox did not become ready on ${SERVICE_HOST}:${SERVICE_PORT}" >&2
exit 1
EOF
