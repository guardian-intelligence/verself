...
artifact_root="${VERIFICATION_ARTIFACT_ROOT:-${VERIFICATION_REPO_ROOT}/artifacts/profile-proof}"
artifact_dir="${artifact_root}/${run_id}"
run_json_path="${artifact_dir}/run.json"
profile_log_path="${artifact_dir}/profile-ui.log"
base_url="${TEST_BASE_URL:-https://rentasandbox.${VERIFICATION_DOMAIN}}"
...
(
  ...
  ./scripts/clickhouse.sh \
    ...
    --query "
      ...
      FROM otel_traces
      WHERE Timestamp BETWEEN parseDateTime64BestEffort({window_start:String}) AND parseDateTime64BestEffort({window_end:String}) + INTERVAL 45 SECOND
        AND ServiceName IN ('profile-service', 'identity-service', 'rent-a-sandbox')
        AND (
