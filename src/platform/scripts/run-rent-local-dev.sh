...
fetch_dev_client_id() {
  ...
  response="$(
    curl -fsS \
      ...
      -d '{"queries":[{"nameQuery":{"name":"rent-a-sandbox-dev","method":"TEXT_QUERY_METHOD_EQUALS"}}]}' \
...
local_profile_port="$(choose_local_port "${RENT_DEV_LOCAL_PROFILE_PORT:-}" 14258 24258 34258 44258 54258)"
...
export RENT_DEV_LOCAL_APP_PORT=${RENT_DEV_LOCAL_APP_PORT}
