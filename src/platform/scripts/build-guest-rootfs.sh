...
if [[ ! -f "$VM_GUEST_TELEMETRY_SRC" ]]; then
  if command -v zig >/dev/null 2>&1 && [[ -f "$PROJECT_ROOT/../vm-guest-telemetry/build.zig" ]]; then
    echo "→ building vm-guest-telemetry from source"
    (
...
VM_GUEST_TELEMETRY_PRESENT=false
VM_GUEST_TELEMETRY_SHA256=""
VM_GUEST_TELEMETRY_BYTES=0
if [[ -f "$ROOTFS/usr/local/bin/vm-guest-telemetry" ]]; then
...
jq -n \
  ...
  --arg vm_guest_telemetry_present "$VM_GUEST_TELEMETRY_PRESENT" \
  ...
  '{
    ...
    vm_guest_telemetry_present: ($vm_guest_telemetry_present == "true"),
    vm_guest_telemetry_sha256: (if $vm_guest_telemetry_sha256 == "" then null else $vm_guest_telemetry_sha256 end),
    vm_guest_telemetry_bytes: (if $vm_guest_telemetry_present == "true" then ($vm_guest_telemetry_bytes | tonumber) else null end)
