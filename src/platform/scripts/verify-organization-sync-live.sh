...
run_id="${VERIFICATION_RUN_ID:-organization-sync-smoke-test-$(date -u +%Y%m%dT%H%M%SZ)}"
artifact_root="${VERIFICATION_ARTIFACT_ROOT:-${VERIFICATION_SMOKE_ARTIFACT_ROOT}/organization-sync-smoke-test}"
...
printf 'organization sync smoke test passed for org %s member %s (%s)\n' "${org_id}" "${target_user_id}" "${target_email}"
