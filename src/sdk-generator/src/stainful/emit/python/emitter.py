...
'''
    ...
    def _auth_args(self, scheme: SecurityScheme | None) -> tuple[str, str, str]:
        """Return (env_name, option_name, super_auth_kwarg)."""
        if scheme is None:
            return "API_KEY", "api_key", "auth_query={}"
        option = snake(scheme.name)
        env = scheme.read_env or f"{self.pkg.upper()}_API_KEY"
        if scheme.location == "query":
            return env, option, (
                f'auth_query={{"{scheme.param_name or "key"}": {option}}}'
            )
        if scheme.kind.startswith("http"):
            return env, option, (
                f'auth_headers={{"Authorization": f"Bearer {{{option}}}"}}'
            )
        return env, option, (
            f'auth_headers={{"{scheme.param_name or "X-API-Key"}": {option}}}'
        )
    ...
    def _client_src(self) -> str:
        ...
        env, auth_option, auth_kwarg = self._auth_args(scheme)
    ...
    def __init__(
        ...
        *,
        ...
        if {auth_option} is None:
            {auth_option} = os.environ.get("{env}")
        if {auth_option} is None:
            raise ValueError(
                "The {auth_option} client option must be set either by passing {auth_option} "
...
'''
    ...
    def _generated_pyproject(self) -> str:
        return (
            ...
            f'dependencies = ["httpx==0.28.1", "pydantic==2.13.4"]\n\n'
