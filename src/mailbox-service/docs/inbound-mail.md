...
- `src/cue-renderer/instances/prod/topology.cue` (`topology.nftables.rulesets.stalwart`) — authoritative egress firewall policy
- `.cache/render/<site>/share/rendered/etc/nftables.d/stalwart.nft` — rendered Stalwart nftables contract (generated by `aspect render --site=<site>`, copied to `/etc/nftables.d/` by the role)
