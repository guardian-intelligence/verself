...
const (
	...
	billingClockTarget    = "//src/billing-service/cmd/billing-clock:billing-clock"
	billingClockBin       = "bazel-bin/src/billing-service/cmd/billing-clock/billing-clock_/billing-clock"
	billingSeedTarget     = "//src/billing-service/cmd/billing-seed:billing-seed"
	billingSeedBin        = "bazel-bin/src/billing-service/cmd/billing-seed/billing-seed_/billing-seed"
)
...
func printBillingPG(rt *opruntime.Runtime, conn *pgx.Conn, opts *billingInspectOptions, orgID, kind string) error {
	...
	if kind == "state" {
		fmt.Fprintf(os.Stdout, "== billing state ==\norg_id=%s\nproduct_id=%s\n\n", orgID, productID)
	}
