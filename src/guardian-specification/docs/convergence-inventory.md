...
| PostgreSQL setup info decode | setup prestart crashes in `backup_repository_status` with `UnicodeDecodeError: 'utf-8' codec can't decode byte 0xc5` while `json.loads`-ing a `text=True` capture of `postgresql-recovery --action=info` | the recovery binary's pgBackRest `info` output against the fresh R2 repo contains a non-utf8 byte, so the strict-utf8 capture crashes before JSON parsing. Reachable for the first time only after the role/secret/R2 blockers were cleared (account-admin imported, R2 recovery produced) | open: the recovery binary must emit valid utf-8 JSON on stdout (or the wrapper must capture bytes and surface decode failures as structured errors) — a postgresql-recovery component fix, not a deploy-path issue |
