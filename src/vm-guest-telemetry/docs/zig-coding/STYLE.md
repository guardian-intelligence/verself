...
working on vm-guest-telemetry, grep through that codebase (`rg -t zig <pattern> docs/references/tigerbeetle/src/`)
...
- All memory must be statically allocated at startup. **No memory may be dynamically allocated (or
  ...
  vm-guest-telemetry has two trust domains with different constraints:
...
vm-guest-telemetry communicates over vsock using fixed-size binary frames. The wire format is the most
...
- Don't overload names with multiple meanings that are context-dependent. For example, vm-guest-telemetry
...
with `zig test docs/zig-coding/examples/<file>.zig` from the `vm-guest-telemetry/` directory.
