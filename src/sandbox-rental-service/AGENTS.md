...
- Zitadel authenticates the Forge Metal user or service account configuring an
  integration.
- GitHub authenticates webhook and installation facts with provider-native
  mechanisms. Webhooks are not Zitadel-authenticated requests.

The durable tenant binding is the GitHub App installation row:
...
github_workflow_jobs
  github_job_id, installation_id, repository_id, labels, runner identity
      |
      | capacity reconcile joins github_installations by installation_id
      v
github_runner_allocations
  allocation_id, installation_id, requested_for_github_job_id
      |
      | AllocateRunner calls Service.Submit with allocation.OrgID
      v
executions
  org_id, actor_id = github-app:<installation_id>,
  source_kind = github_actions, workload_kind = github_runner,
  external_provider = github, external_task_id = <github_job_id>
      |
...
- The GitHub callback is safe only because `state` was generated by
  ...
  installation.
- `workflow_job` webhooks create demand facts. They do not directly create
...
- Billing is keyed to our execution system of record. `billing_windows.source_ref`
  is the Forge Metal `execution_id`; the GitHub job ID is provider correlation
  metadata on `executions.external_task_id` and the GitHub tables.
...
- Webhook demand ingestion: `internal/jobs/github_runner.go:HandleWebhook`.
- Org recovery for GitHub demand: `internal/jobs/github_runner.go:loadQueuedJob`
  joins `github_workflow_jobs` to `github_installations`.
- Execution submission from runner allocation:
  `internal/jobs/github_runner.go:AllocateRunner`.
