...
The current HTTP API is the authenticated human inbox API generated by Huma in
`internal/api/routes.go` and DTOs in
`src/domain-transfer-objects/go/notifications.go`.
...
The next API should copy Knock's workflow-trigger shape. Callers trigger a named
...
{
  "tenant": "org_123",
  "recipients": ["user:user_456"],
  "actor": "system:sandbox-rental-service",
  "data": {
    "repository": "guardian-intelligence/verself",
    "workflow": "ci",
    "status": "failed",
    "resource_kind": "source_workflow_run",
    "resource_id": "018f...",
    "action_url": "https://verself.sh/source/runs/018f...",
    "trace_id": "e3a..."
  },
  "cancellation_key": "runner:workflow:018f..."
}
```
...
| `tenant` | Required Verself org id. This is the tenant boundary used for preferences and governance. |
| `recipients` | Required recipient identifiers. Supported initial forms are `user:<subject_id>`, `contact:<contact_id>`, and `org_role:<role>`. |
| `actor` | Optional user, service, or system actor. Service actors use `system:<service-name>`. |
| `data` | Workflow data object. Validated against the workflow schema before persistence. |
| `cancellation_key` | Optional producer-scoped key for canceling pending delayed/digested delivery later. |
| `traceparent` | Optional W3C trace context; also accepted from the HTTP header. |

The internal Go SDK should expose the generated client plus a small curated
helper:

```go
run, err := notifications.Trigger(ctx, "runner.workflow_failed", notifications.TriggerRequest{
    Tenant:     orgID,
    Recipients: notifications.Users(userID),
    Actor:      notifications.System("sandbox-rental-service"),
...
}, notifications.WithIdempotencyKey("runner:workflow-failed:"+workflowRunID))
```

High-volume product paths may add typed helpers, but helpers compile down to the
same `Trigger` command. Producers never choose email addresses, SMTP providers,
inbox rows, or notification templates.
