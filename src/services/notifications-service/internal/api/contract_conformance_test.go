...
func TestPublicOpenAPIConformsToContract(t *testing.T) {
	api := NewAPI(http.NewServeMux(), Config{Version: "test", ListenAddr: "https://notifications.api.verself.test", Service: &notifications.Service{}, InstallationID: "inst_openapi"})
...
func TestInternalOpenAPIConformsToContract(t *testing.T) {
	api := NewInternalAPI(http.NewServeMux(), "test", "https://127.0.0.1:4254", InternalConfig{Service: &notifications.Service{}})
...
func assertPublicOperationConforms(t *testing.T, path string, op *huma.Operation, want contractapi.OperationDescriptor) {
	...
	if want.Idempotency.Policy == string(runtimeiam.IdempotencyHeaderKey) && !operationHasRequiredParameter(op, "header", "Idempotency-Key") {
...
func assertOperationTransportConforms(t *testing.T, path string, op *huma.Operation, method string, contractPath string, defaultStatus int, bodyBudget int64) {
	...
	if bodyBudget > 0 && op.MaxBodyBytes != bodyBudget {
...
func publicContractOperationsByID(t *testing.T) map[string]contractapi.OperationDescriptor {
	...
	if len(out) == 0 {
		t.Fatal("generated notifications public contract is empty")
	}
	return out
}
...
func internalContractOperationsByID(t *testing.T) map[string]internalcontractapi.OperationDescriptor {
	...
	if len(out) == 0 {
		t.Fatal("generated notifications internal contract is empty")
	}
	return out
}
