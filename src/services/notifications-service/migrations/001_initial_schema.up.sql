CREATE TABLE notification_inbox_state (
    ...
    PRIMARY KEY (inbox_state_id),
    UNIQUE (org_id, recipient_subject_id),
...
CREATE INDEX notification_workflow_runs_org_idx ON notification_workflow_runs (org_id, created_at DESC);
...
CREATE INDEX notification_delivery_attempts_pending_idx ON notification_delivery_attempts (next_attempt_at, queued_at)
    WHERE status IN ('queued', 'failed');
CREATE INDEX notification_delivery_attempts_workflow_idx ON notification_delivery_attempts (workflow_run_id);
