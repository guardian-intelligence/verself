...
- `mailbox_service` (user `mailbox_service`) — repo-owned normalized mailbox projection and auth binding state. The schema is managed in `src/services/mailbox-service/migrations/` and currently contains `mailbox_accounts`, `mailbox_bindings`, `sync_state`, `mailboxes`, `emails`, `email_mailboxes`, `email_bodies`, and `threads`.
...
- `src/components/stalwart/nftables/stalwart.nft` — Stalwart nftables contract referenced by `src/configuration` and copied to `/etc/nftables.d/` by host firewall convergence
