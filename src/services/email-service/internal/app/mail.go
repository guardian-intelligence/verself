...
import (
	...
	"github.com/verself/email-service/internal/jmap"
...
func (s *Service) SetEmailSeen(ctx context.Context, accountID, emailID string, seen bool) error {
	...
	slog.InfoContext(ctx, "email-service: email keywords patched",
...
func (s *Service) MoveEmail(ctx context.Context, accountID, emailID, mailboxID string) error {
	current, client, account, err := s.currentEmail(ctx, accountID, emailID)
	if err != nil {
		slog.ErrorContext(ctx, "email-service: move email current state lookup failed",
			"mailbox_account", accountID,
			"email_id", emailID,
	...
	slog.InfoContext(ctx, "email-service: move email requested",
		"mailbox_account", accountID,
		"email_id", emailID,
	...
	if err != nil {
		slog.ErrorContext(ctx, "email-service: move email upstream mutation failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"target_mailbox_id", mailboxID,
			"error", err,
	...
	if err := s.store.ReplaceEmailMailboxes(ctx, accountID, emailID, []string{mailboxID}); err != nil {
		slog.ErrorContext(ctx, "email-service: move email local mailbox patch failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"target_mailbox_id", mailboxID,
			"error", err,
	...
	snapshot, err := s.store.GetEmailSnapshot(ctx, accountID, emailID)
	if err != nil {
		slog.ErrorContext(ctx, "email-service: move email snapshot reload failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"target_mailbox_id", mailboxID,
			"error", err,
	...
	slog.InfoContext(ctx, "email-service: email mailboxes patched",
		"mailbox_account", accountID,
		"email_id", emailID,
...
func (s *Service) TrashEmail(ctx context.Context, accountID, emailID string) error {
	current, client, account, err := s.currentEmail(ctx, accountID, emailID)
	if err != nil {
		slog.ErrorContext(ctx, "email-service: trash email current state lookup failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"error", err,
		)
	...
	if err != nil {
		slog.ErrorContext(ctx, "email-service: trash mailbox lookup failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"error", err,
		)
	...
	if err != nil {
		slog.ErrorContext(ctx, "email-service: trash email upstream mutation failed",
			"mailbox_account", accountID,
			"email_id", emailID,
	...
	if err := s.store.ReplaceEmailMailboxes(ctx, accountID, emailID, []string{trashMailboxID}); err != nil {
		slog.ErrorContext(ctx, "email-service: trash email local mailbox patch failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"trash_mailbox_id", trashMailboxID,
			"error", err,
	...
	snapshot, err := s.store.GetEmailSnapshot(ctx, accountID, emailID)
	if err != nil {
		slog.ErrorContext(ctx, "email-service: trash email snapshot reload failed",
			"mailbox_account", accountID,
			"email_id", emailID,
			"trash_mailbox_id", trashMailboxID,
			"error", err,
