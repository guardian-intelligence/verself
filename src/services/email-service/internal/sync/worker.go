...
func (w *Worker) Run(ctx context.Context) {
	...
	for {
		...
		if err != nil {
			w.setError(err)
			w.cfg.Logger.ErrorContext(ctx, "email-service: sync worker client init failed", "error", err)
		...
		if err := w.ensureSynced(ctx, client, principal); err != nil {
			w.setError(err)
			w.cfg.Logger.ErrorContext(ctx, "email-service: sync worker bootstrap failed", "error", err)
			if !sleepContext(ctx, 5*time.Second) {
				return
			}
			continue
		...
		if err != nil {
			w.setError(err)
			w.cfg.Logger.ErrorContext(ctx, "email-service: sync worker eventsource connect failed", "error", err)
			if !sleepContext(ctx, 5*time.Second) {
				return
			}
			continue
		...
		w.cfg.Logger.InfoContext(ctx, "email-service: sync worker eventsource connected")
		...
		for {
			...
			case event, ok := <-eventCh:
				...
				if err := w.handleEvent(ctx, client, principal, event); err != nil {
					w.setError(err)
					w.cfg.Logger.ErrorContext(ctx, "email-service: sync worker event application failed", "error", err)
				}
			case err := <-errCh:
				reconcileTicker.Stop()
				_ = stream.Close()
				...
				if err != nil && !errors.Is(err, io.EOF) && !errors.Is(err, context.Canceled) {
					w.setError(err)
					w.cfg.Logger.ErrorContext(ctx, "email-service: sync worker eventsource disconnected", "error", err)
				}
				break loop
			case <-reconcileTicker.C:
				if err := w.applyPendingChanges(ctx, client, principal, "reconcile"); err != nil {
					w.setError(err)
					w.cfg.Logger.ErrorContext(ctx, "email-service: sync worker reconcile failed", "error", err)
				}
			}
		}
