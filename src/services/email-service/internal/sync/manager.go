...
func (m *Manager) Run(ctx context.Context) {
	...
	if err := m.reconcileWorkers(ctx); err != nil {
		m.setError(err)
		m.logger.ErrorContext(ctx, "email-service: mailbox sync discovery failed", "error", err)
	}
...
func (m *Manager) reconcileWorkers(ctx context.Context) error {
	...
	for accountID, principal := range seen {
		...
		password, err := m.mailboxPassword(accountID)
		if err != nil {
			m.status.Accounts[accountID] = AccountStatus{
				AccountID: accountID,
				LastError: err.Error(),
			}
			m.logger.WarnContext(ctx, "email-service: mailbox sync skipped account without protocol credential", "mailbox_account", accountID)
			continue
		}
