...
func TestResolveStickyDiskPathUsesGitHubWorkspace(t *testing.T) {
	got, err := resolveStickyDiskPath("src/websites/node_modules", "guardian-intelligence/verself")
	if err != nil {
		t.Fatalf("resolveStickyDiskPath returned error: %v", err)
	}
	want := "/workspace/verself/verself/src/websites/node_modules"
	if got != want {
    ...
    steps:
      - uses: guardian-intelligence/verself/.github/actions/setup-node@main
        with:
          ...
          working-directory: src/websites
          cache: true
          node-modules: true
`)
	files := map[string][]byte{
		"src/websites/pnpm-lock.yaml": []byte("lockfileVersion: '9.0'\n"),
		"src/websites/package.json":   []byte(`{"packageManager":"pnpm@10.33.0"}`),
	}
	...
	lockHash := sha256Hex(files["src/websites/pnpm-lock.yaml"])
	wantStoreKey := "setup-node:v1:repo=guardian-intelligence/verself:runner=verself-4vcpu-ubuntu-2404:node=24:pm=pnpm@10.33.0:workdir=src/websites:lock=" + lockHash + ":store"
	wantModulesKey := "setup-node:v1:repo=guardian-intelligence/verself:runner=verself-4vcpu-ubuntu-2404:node=24:pm=pnpm@10.33.0:workdir=src/websites:lock=" + lockHash + ":node_modules"
	if decls[0].Key != wantStoreKey || decls[0].Path != "~/.pnpm-store" {
		t.Fatalf("store declaration = %#v", decls[0])
	}
	if decls[1].Key != wantModulesKey || decls[1].Path != "src/websites/node_modules" {
