...
var (
	...
	runtimeSecretReferenceRE = regexp.MustCompile(`kv-runtime/data/secret/org/([a-z][a-z0-9-]*(?:\.[a-z0-9_]+)+)`)
)
...
func LoadBundle(repoRoot, site string, components []Component) (Bundle, error) {
	...
	if err := validateNomadSecretReferences(repoRoot, components, secrets); err != nil {
		return Bundle{}, err
	}
...
func BundleSHA256(bundle Bundle) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("encode control-plane bundle: %w", err)
	}
...
func validateNomadSecretReferences(repoRoot string, components []Component, secrets []RuntimeSecret) error {
	secretByName := map[string]RuntimeSecret{}
	for _, secret := range secrets {
		secretByName[secret.Name] = secret
	}
	for _, component := range components {
		jobID := strings.TrimSpace(component.JobID)
		if jobID == "" {
			return fmt.Errorf("component job id is required for runtime secret reference validation")
		}
		path := resolvePath(repoRoot, component.JobSpec)
		body, err := os.ReadFile(path)
		if err != nil {
			return fmt.Errorf("%s: read Nomad job spec for runtime secret reference validation: %w", component.JobSpec, err)
		}
		references := runtimeSecretReferences(body)
		for _, name := range references {
			secret, ok := secretByName[name]
			if !ok {
				return fmt.Errorf("%s: Nomad job %q references undeclared OpenBao runtime secret %q", component.JobSpec, jobID, name)
			}
			if !runtimeSecretReadableBy(secret, jobID) {
				return fmt.Errorf("%s: Nomad job %q references OpenBao runtime secret %q but is neither owner job %q nor declared in consumer_job_ids", component.JobSpec, jobID, name, secret.JobID)
			}
		}
	}
	return nil
...
func runtimeSecretReferences(body []byte) []string {
	matches := runtimeSecretReferenceRE.FindAllSubmatch(body, -1)
...
func runtimeSecretReadableBy(secret RuntimeSecret, jobID string) bool {
	if secret.JobID == jobID {
		return true
	}
	for _, consumerJobID := range secret.ConsumerJobIDs {
		if consumerJobID == jobID {
			return true
		}
	}
	return false
...
func walkDeployFiles(repoRoot string, visit func(path, rel string) error) error {
	for _, relRoot := range []string{"src/infrastructure-components", "src/integrations", "src/services", "src/viteplus-monorepo/apps"} {
