...
func ApplyBundleFile(ctx context.Context, cfg ApplyConfig) (ApplyResult, error) {
	...
	result.ProducedSecrets = producedSecrets
	return result, nil
}
...
func writeProducedRuntimeSecrets(ctx context.Context, c *baoClient, bundle Bundle) (int, error) {
	...
	for name, value := range values {
		existing, found, err := c.readRuntimeSecret(ctx, name)
		if err != nil {
			return 0, err
		}
		...
		if err := c.writeRuntimeSecret(ctx, name, value); err != nil {
			return 0, fmt.Errorf("write produced OpenBao runtime secret %s: %w", name, err)
		}
	}
	return len(values), nil
}

func substrateControlPlaneProducedSecretValues(bundle Bundle) map[string]string {
	...
	for _, secret := range bundle.OpenBao.RuntimeSecrets {
		...
		out[secret.Name] = "verself.substrate-control-plane.applied.v1:" + bundle.Site
	}
	return out
}
