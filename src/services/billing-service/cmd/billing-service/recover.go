...
const (
	defaultBillingRuntimeRoot    = "/var/lib/billing-service/runtime"
	defaultBillingProjectedGraph = "/run/verself/recovery/billing-service/document.json"
	billingServiceUser           = "billing"
	billingServiceUID            = 991
	billingServiceGID            = 984
	billingSPIREWorkloadGroup    = "spire_workload"
	billingServiceStateRoot      = "/var/lib/billing-service"
	billingServiceHome           = "/var/lib/billing-service/home"
	billingBoardedBinaryRelative = "bazel-bin/src/services/billing-service/cmd/billing-service/billing-service_/billing-service"
)
...
func runRecoveryCLI(ctx context.Context, args []string) (bool, error) {
	...
	return true, recoverBillingService(ctx, opts)
}
...
func parseRecoverOptions(args []string) (recoverOptions, error) {
	opts := recoverOptions{
		...
		RuntimeRoot:    defaultBillingRuntimeRoot,
		ProjectedGraph: defaultBillingProjectedGraph,
	}
	...
	fs.StringVar(&opts.RuntimeRoot, "runtime-root", opts.RuntimeRoot, "billing-service runtime install root")
	fs.StringVar(&opts.ProjectedGraph, "projected-graph", opts.ProjectedGraph, "runtime-readable Guardian resource graph projection")
	fs.BoolVar(&opts.Migrate, "migrate", opts.Migrate, "run billing-service database migrations after runtime installation")
...
func recoverBillingService(ctx context.Context, opts recoverOptions) error {
	if os.Geteuid() != 0 {
		return errors.New("billing-service recover must run as root")
	}
	service, err := ensureLocalIdentity(billingServiceUser)
	if err != nil {
		return err
	}
	sourceBinary := filepath.Join(opts.RepoRoot, billingBoardedBinaryRelative)
	if err := installRuntime(opts.RuntimeRoot, sourceBinary, serviceName, service.gid); err != nil {
		return err
	}
	if err := projectResourceGraph(opts.ResourceGraph, opts.ProjectedGraph, serviceName, service.gid); err != nil {
		return err
	}
	runtimeCfg, err := loadBillingRuntimeConfig(opts.ProjectedGraph, opts.ResourceName)
	if err != nil {
		return err
	}
...
func ensureLocalIdentity(name string) (localIdentity, error) {
	if err := ensureLocalGroup(name, billingServiceGID); err != nil {
		return localIdentity{}, err
	}
	if _, err := user.LookupGroup(billingSPIREWorkloadGroup); err != nil {
		return localIdentity{}, fmt.Errorf("lookup %s group: %w", billingSPIREWorkloadGroup, err)
	}
	if _, err := user.Lookup(name); err == nil {
		if err := reconcileLocalUser(name, billingServiceUID, billingServiceGID); err != nil {
			return localIdentity{}, err
		}
		...
		if err := runRootCommand(
			"/usr/sbin/useradd",
			"--system",
			"--uid", strconv.Itoa(billingServiceUID),
			"--gid", name,
			"--groups", billingSPIREWorkloadGroup,
			"--home-dir", billingServiceHome,
	...
	if err := ensureSupplementaryGroup(name, billingSPIREWorkloadGroup); err != nil {
		return localIdentity{}, err
	}
	...
	if identity.uid != billingServiceUID || identity.gid != billingServiceGID {
		return localIdentity{}, fmt.Errorf("%s has uid:gid %d:%d, want %d:%d", name, identity.uid, identity.gid, billingServiceUID, billingServiceGID)
	}
	return identity, nil
}
...
func reconcileLocalUser(name string, uid int, gid int) error {
	...
	if err := runRootCommand("/usr/sbin/usermod", "--home", billingServiceHome, "--shell", "/usr/sbin/nologin", name); err != nil {
		return err
	}
	return nil
}
...
func ensureStateDirs(service localIdentity) error {
	if err := ensureOwnedDir(billingServiceStateRoot, 0, 0, 0o755); err != nil {
		return err
	}
	for _, dir := range []string{
		billingServiceHome,
		filepath.Join(billingServiceHome, "tmp"),
	} {
