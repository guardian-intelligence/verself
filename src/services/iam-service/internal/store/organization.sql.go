...
SELECT org_id, identity_provider_org_id, display_name, slug, version
FROM iam_organizations
WHERE identity_provider_org_id = ANY($1::text[])
ORDER BY display_name, org_id
`
...
SELECT EXISTS (
    SELECT 1 FROM iam_organizations o WHERE o.slug = $1 AND o.org_id <> $2
) OR EXISTS (
    SELECT 1 FROM iam_organization_slug_redirects r WHERE r.slug = $1
) AS unavailable
`
