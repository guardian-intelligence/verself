...
const (
	defaultSourceRuntimeRoot    = "/var/lib/source-code-hosting-service/runtime"
	defaultSourceProjectedGraph = "/run/verself/recovery/source-code-hosting-service/document.json"
	sourceServiceUser           = "source_code_hosting_service"
	sourceServiceUID            = 974
	sourceServiceGID            = 967
	sourceSPIREWorkloadGroup    = "spire_workload"
	sourceServiceStateRoot      = "/var/lib/source-code-hosting-service"
	sourceServiceHome           = "/var/lib/source-code-hosting-service/home"
	sourceBoardedBinaryRelative = "bazel-bin/src/services/source-code-hosting-service/cmd/source-code-hosting-service/source-code-hosting-service_/source-code-hosting-service"
)
...
func runRecoveryCLI(ctx context.Context, args []string) (bool, error) {
	...
	return true, recoverSourceCodeHostingService(ctx, opts)
}
...
func parseRecoverOptions(args []string) (recoverOptions, error) {
	opts := recoverOptions{
		...
		RuntimeRoot:    defaultSourceRuntimeRoot,
		ProjectedGraph: defaultSourceProjectedGraph,
	}
	fs := flag.NewFlagSet("source-code-hosting-service recover", flag.ContinueOnError)
	...
	fs.StringVar(&opts.RuntimeRoot, "runtime-root", opts.RuntimeRoot, "source-code-hosting-service runtime install root")
	fs.StringVar(&opts.ProjectedGraph, "projected-graph", opts.ProjectedGraph, "runtime-readable Guardian resource graph projection")
	fs.BoolVar(&opts.Migrate, "migrate", opts.Migrate, "run source-code-hosting-service database migrations after runtime installation")
...
func recoverSourceCodeHostingService(ctx context.Context, opts recoverOptions) error {
	if os.Geteuid() != 0 {
		return errors.New("source-code-hosting-service recover must run as root")
	}
	service, err := ensureLocalIdentity(sourceServiceUser)
	if err != nil {
		return err
	}
	sourceBinary := filepath.Join(opts.RepoRoot, sourceBoardedBinaryRelative)
	if err := installRuntime(opts.RuntimeRoot, sourceBinary, "source-code-hosting-service", service.gid); err != nil {
		return err
	}
	if err := projectResourceGraph(opts.ResourceGraph, opts.ProjectedGraph, "source-code-hosting-service", service.gid); err != nil {
		return err
	}
	runtimeCfg, err := loadSourceRuntimeConfig(opts.ProjectedGraph, opts.ResourceName)
	if err != nil {
		return err
	}
...
func ensureLocalIdentity(name string) (localIdentity, error) {
	if err := ensureLocalGroup(name, sourceServiceGID); err != nil {
		return localIdentity{}, err
	}
	if _, err := user.LookupGroup(sourceSPIREWorkloadGroup); err != nil {
		return localIdentity{}, fmt.Errorf("lookup %s group: %w", sourceSPIREWorkloadGroup, err)
	}
	if _, err := user.Lookup(name); err == nil {
		if err := reconcileLocalUser(name, sourceServiceUID, sourceServiceGID); err != nil {
			return localIdentity{}, err
		}
		...
		if err := runRootCommand(
			"/usr/sbin/useradd",
			"--system",
			"--uid", strconv.Itoa(sourceServiceUID),
			"--gid", name,
			"--groups", sourceSPIREWorkloadGroup,
			"--home-dir", sourceServiceHome,
	...
	if err := ensureSupplementaryGroup(name, sourceSPIREWorkloadGroup); err != nil {
		return localIdentity{}, err
	}
	...
	if identity.uid != sourceServiceUID || identity.gid != sourceServiceGID {
		return localIdentity{}, fmt.Errorf("%s has uid:gid %d:%d, want %d:%d", name, identity.uid, identity.gid, sourceServiceUID, sourceServiceGID)
	}
	return identity, nil
}
...
func reconcileLocalUser(name string, uid int, gid int) error {
	...
	if err := runRootCommand("/usr/sbin/usermod", "--home", sourceServiceHome, "--shell", "/usr/sbin/nologin", name); err != nil {
		return err
	}
	return nil
}
...
func ensureStateDirs(service localIdentity) error {
	if err := ensureOwnedDir(sourceServiceStateRoot, 0, 0, 0o755); err != nil {
		return err
	}
	dirs := []string{
		sourceServiceHome,
		filepath.Join(sourceServiceHome, "tmp"),
	}
