...
const (
	defaultProjectsRuntimeRoot    = "/var/lib/projects-service/runtime"
	defaultProjectsProjectedGraph = "/run/verself/recovery/projects-service/document.json"
	projectsServiceUser           = "projects_service"
	projectsServiceUID            = 977
	projectsServiceGID            = 970
	projectsSPIREWorkloadGroup    = "spire_workload"
	projectsServiceStateRoot      = "/var/lib/projects-service"
	projectsServiceHome           = "/var/lib/projects-service/home"
	projectsBoardedBinaryRelative = "bazel-bin/src/services/projects-service/cmd/projects-service/projects-service_/projects-service"
)
...
func runRecoveryCLI(ctx context.Context, args []string) (bool, error) {
	...
	return true, recoverProjectsService(ctx, opts)
}
...
func parseRecoverOptions(args []string) (recoverOptions, error) {
	opts := recoverOptions{
		...
		RuntimeRoot:    defaultProjectsRuntimeRoot,
		ProjectedGraph: defaultProjectsProjectedGraph,
	}
	fs := flag.NewFlagSet("projects-service recover", flag.ContinueOnError)
	...
	fs.StringVar(&opts.RuntimeRoot, "runtime-root", opts.RuntimeRoot, "projects-service runtime install root")
	fs.StringVar(&opts.ProjectedGraph, "projected-graph", opts.ProjectedGraph, "runtime-readable Guardian resource graph projection")
	fs.BoolVar(&opts.Migrate, "migrate", opts.Migrate, "run projects-service database migrations after runtime installation")
...
func recoverProjectsService(ctx context.Context, opts recoverOptions) error {
	if os.Geteuid() != 0 {
		return errors.New("projects-service recover must run as root")
	}
	service, err := ensureLocalIdentity(projectsServiceUser)
	if err != nil {
		return err
	}
	sourceBinary := filepath.Join(opts.RepoRoot, projectsBoardedBinaryRelative)
	if err := installRuntime(opts.RuntimeRoot, sourceBinary, "projects-service", service.gid); err != nil {
		return err
	}
	if err := projectResourceGraph(opts.ResourceGraph, opts.ProjectedGraph, "projects-service", service.gid); err != nil {
		return err
	}
	runtimeCfg, err := loadProjectsRuntimeConfig(opts.ProjectedGraph, opts.ResourceName)
	if err != nil {
		return err
	}
...
func ensureLocalIdentity(name string) (localIdentity, error) {
	if err := ensureLocalGroup(name, projectsServiceGID); err != nil {
		return localIdentity{}, err
	}
	if _, err := user.LookupGroup(projectsSPIREWorkloadGroup); err != nil {
		return localIdentity{}, fmt.Errorf("lookup %s group: %w", projectsSPIREWorkloadGroup, err)
	}
	if _, err := user.Lookup(name); err == nil {
		if err := reconcileLocalUser(name, projectsServiceUID, projectsServiceGID); err != nil {
			return localIdentity{}, err
		}
		...
		if err := runRootCommand(
			"/usr/sbin/useradd",
			"--system",
			"--uid", strconv.Itoa(projectsServiceUID),
			"--gid", name,
			"--groups", projectsSPIREWorkloadGroup,
			"--home-dir", projectsServiceHome,
	...
	if err := ensureSupplementaryGroup(name, projectsSPIREWorkloadGroup); err != nil {
		return localIdentity{}, err
	}
	...
	if identity.uid != projectsServiceUID || identity.gid != projectsServiceGID {
		return localIdentity{}, fmt.Errorf("%s has uid:gid %d:%d, want %d:%d", name, identity.uid, identity.gid, projectsServiceUID, projectsServiceGID)
	}
	return identity, nil
}
...
func reconcileLocalUser(name string, uid int, gid int) error {
	...
	if err := runRootCommand("/usr/sbin/usermod", "--home", projectsServiceHome, "--shell", "/usr/sbin/nologin", name); err != nil {
		return err
	}
	return nil
}
...
func ensureStateDirs(service localIdentity) error {
	if err := ensureOwnedDir(projectsServiceStateRoot, 0, 0, 0o755); err != nil {
		return err
	}
	dirs := []string{
		projectsServiceHome,
		filepath.Join(projectsServiceHome, "tmp"),
	}
