...
const (
	...
	profileServiceUID            = 978
	profileServiceGID            = 971
	...
	profileBoardedBinaryRelative = "bazel-bin/src/services/profile-service/cmd/profile-service/profile-service_/profile-service"
)
...
func runRecoveryCLI(ctx context.Context, args []string) (bool, error) {
	...
	return true, recoverProfileService(ctx, opts)
}
...
func parseRecoverOptions(args []string) (recoverOptions, error) {
	...
	fs.StringVar(&opts.RuntimeRoot, "runtime-root", opts.RuntimeRoot, "profile-service runtime install root")
	fs.StringVar(&opts.ProjectedGraph, "projected-graph", opts.ProjectedGraph, "runtime-readable Guardian resource graph projection")
	fs.BoolVar(&opts.Migrate, "migrate", opts.Migrate, "run profile-service database migrations after runtime installation")
...
func recoverProfileService(ctx context.Context, opts recoverOptions) error {
	if os.Geteuid() != 0 {
	...
	sourceBinary := filepath.Join(opts.RepoRoot, profileBoardedBinaryRelative)
	if err := installRuntime(opts.RuntimeRoot, sourceBinary, "profile-service", service.gid); err != nil {
		return err
	}
	if err := projectResourceGraph(opts.ResourceGraph, opts.ProjectedGraph, "profile-service", service.gid); err != nil {
		return err
	}
	runtimeCfg, err := loadProfileRuntimeConfig(opts.ProjectedGraph, opts.ResourceName)
	if err != nil {
		return err
	}
...
func ensureLocalIdentity(name string) (localIdentity, error) {
	if err := ensureLocalGroup(name, profileServiceGID); err != nil {
	...
	if _, err := user.Lookup(name); err == nil {
		if err := reconcileLocalUser(name, profileServiceUID, profileServiceGID); err != nil {
			return localIdentity{}, err
		}
		...
		if err := runRootCommand(
			"/usr/sbin/useradd",
			"--system",
			"--uid", strconv.Itoa(profileServiceUID),
			"--gid", name,
	...
	if identity.uid != profileServiceUID || identity.gid != profileServiceGID {
		return localIdentity{}, fmt.Errorf("%s has uid:gid %d:%d, want %d:%d", name, identity.uid, identity.gid, profileServiceUID, profileServiceGID)
	}
	return identity, nil
}
...
func reconcileLocalUser(name string, uid int, gid int) error {
	...
	if err := runRootCommand("/usr/sbin/usermod", "--home", profileServiceHome, "--shell", "/usr/sbin/nologin", name); err != nil {
		return err
	}
	return nil
}
