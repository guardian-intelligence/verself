...
const (
	...
	distributionBoardedBinaryRelative = "bazel-bin/src/services/distribution-service/cmd/distribution-service/distribution-service_/distribution-service"
)
...
func runRecoveryCLI(ctx context.Context, args []string) (bool, error) {
	...
	return true, recoverDistributionService(ctx, opts)
}
...
func parseRecoverOptions(args []string) (recoverOptions, error) {
	...
	fs.StringVar(&opts.RuntimeRoot, "runtime-root", opts.RuntimeRoot, "distribution-service runtime install root")
	fs.StringVar(&opts.ProjectedGraph, "projected-graph", opts.ProjectedGraph, "runtime-readable Guardian resource graph projection")
	fs.BoolVar(&opts.Migrate, "migrate", opts.Migrate, "run distribution-service database migrations after runtime installation")
...
func recoverDistributionService(ctx context.Context, opts recoverOptions) error {
	if os.Geteuid() != 0 {
	...
	sourceBinary := filepath.Join(opts.RepoRoot, distributionBoardedBinaryRelative)
	...
	runtimeCfg, err := loadDistributionRuntimeConfig(opts.ProjectedGraph, opts.ResourceName)
	if err != nil {
		return err
	}
