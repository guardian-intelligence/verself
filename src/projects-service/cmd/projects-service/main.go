...
func run() error {
	...
	defer func() {
		if err := otelShutdown(context.Background()); err != nil {
			logger.ErrorContext(context.Background(), "projects-service otel shutdown", "error", err)
		}
	}()
	slog.SetDefault(logger)

	cfg := envconfig.New()
	pgDSN := cfg.RequireString("PROJECTS_PG_DSN")
	listenAddr := cfg.String("PROJECTS_LISTEN_ADDR", "127.0.0.1:4264")
	internalListenAddr := cfg.String("PROJECTS_INTERNAL_LISTEN_ADDR", "127.0.0.1:4265")
	authIssuerURL := cfg.RequireURL("PROJECTS_AUTH_ISSUER_URL")
	authAudience := cfg.RequireString("PROJECTS_AUTH_AUDIENCE")
	pgMaxConns := cfg.Int("PROJECTS_PG_MAX_CONNS", 8)
	...
	defer func() {
		if err := spiffeSource.Close(); err != nil {
			logger.ErrorContext(context.Background(), "projects-service spiffe source close", "error", err)
		}
	}()
	...
	svc := &projects.Service{Store: projects.SQLStore{PG: pg}}
	if err := svc.Ready(ctx); err != nil {
		return fmt.Errorf("projects readiness: %w", err)
	}
