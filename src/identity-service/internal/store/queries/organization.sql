...
WHERE NOT EXISTS (
    SELECT 1 FROM identity_organization_slug_redirects WHERE slug = sqlc.arg(slug)
)
...
SELECT EXISTS (
    SELECT 1 FROM identity_organizations o WHERE o.slug = sqlc.arg(candidate_slug) AND o.org_id <> sqlc.arg(current_org_id)
) OR EXISTS (
    SELECT 1 FROM identity_organization_slug_redirects r WHERE r.slug = sqlc.arg(candidate_slug)
) AS unavailable;
