...
func sendGovernanceAudit(ctx context.Context, record governanceAuditRecord) {
	...
	if err != nil {
		slog.Default().ErrorContext(ctx, "identity governance audit marshal failed", "error", err)
		return
	}
	...
	if err != nil {
		slog.Default().ErrorContext(ctx, "identity governance audit request failed", "error", err)
		return
	}
