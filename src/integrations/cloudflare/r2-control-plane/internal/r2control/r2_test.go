...
func TestPresignGetObjectScopesTemporaryCredentialToURL(t *testing.T) {
	client, err := NewR2Client(R2ClientConfig{
		Endpoint:        Endpoint("c3eaeffaadf7d4847684d4775c16d598"),
		Region:          "auto",
		AccessKeyID:     "access-key",
	...
	signed, err := client.PresignGetObject(t.Context(), "verself-deployment-artifacts", "gamma/sha256/abc/service.tar", 5*time.Minute)
	if err != nil {
		t.Fatal(err)
	}
	u, err := url.Parse(signed)
