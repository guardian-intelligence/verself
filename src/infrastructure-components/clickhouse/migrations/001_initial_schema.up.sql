...
-- ═══════════════════════════════════════════════════════════════════════════
-- verself: governance audit events (tamper-evident authorization ledger)
-- ═══════════════════════════════════════════════════════════════════════════

DROP TABLE IF EXISTS verself.audit_event_details;
...
ORDER BY (org_id, event_date, sequence, event_id)
SETTINGS index_granularity = 8192;

CREATE TABLE verself.audit_event_details
(
    event_id UUID,
    org_id LowCardinality(String),
    event_date Date,
    detail_json String CODEC(ZSTD(6)),
    detail_sha256 String
)
ENGINE = MergeTree
PARTITION BY toYYYYMM(event_date)
ORDER BY (org_id, event_date, event_id)
SETTINGS index_granularity = 8192;
...
-- ═══════════════════════════════════════════════════════════════════════════
-- Deploy controller evidence (database: verself)
-- ═══════════════════════════════════════════════════════════════════════════
