job "haproxy-upstreams" {
  ...
  group "haproxy-upstreams" {
    ...
    task "haproxy-upstreams" {
      ...
      config {
        args = ["--source", "local/nomad-upstreams.cfg", "--dest", "/etc/haproxy/nomad-upstreams.cfg", "--haproxy-bin", "local/bin/haproxy", "--haproxy-config", "/etc/haproxy/haproxy.cfg", "--haproxy-config", "/etc/haproxy/nomad-upstreams.cfg", "--haproxy-ld-library-path", "local/lib", "--reload-unit", "haproxy.service", "--daemon"]
...
EOT
        ...
        change_script {
          command = "local/bin/haproxy-upstreams-apply"
          args = ["--source", "local/nomad-upstreams.cfg", "--dest", "/etc/haproxy/nomad-upstreams.cfg", "--haproxy-bin", "local/bin/haproxy", "--haproxy-config", "/etc/haproxy/haproxy.cfg", "--haproxy-config", "/etc/haproxy/nomad-upstreams.cfg", "--haproxy-ld-library-path", "local/lib", "--reload-unit", "haproxy.service"]
          timeout = "5s"
          fail_on_error = true
        }
      }
