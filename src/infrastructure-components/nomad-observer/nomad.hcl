job "nomad-observer" {
  ...
  group "observer" {
    ...
    task "podman-measurer" {
      ...
      env {
        NOMAD_OBSERVER_MODE = "podman-measurer"
        NOMAD_OBSERVER_PODMAN_MEASURE_GROUP = "nomad_observer"
        NOMAD_OBSERVER_PODMAN_MEASURE_SOCKET = "/run/verself-nomad-observer/podman-measure.sock"
      }
