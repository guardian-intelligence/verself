...
func run() error {
	...
	stripeProductID := stripeProductID(cfg.productID)
	product, err := ensureStripeProduct(ctx, stripeClient, stripeProductID, cfg)
	if err != nil {
		return err
	}
...
func parseFlags() (config, error) {
	cfg := config{
		...
		productDisplayName: defaultProductDisplayName,
		rateSourcePlanID:   defaultRateSourcePlanID,
	}
...
func ensureStripeProduct(ctx context.Context, stripeClient *stripe.Client, stripeProductID string, cfg config) (*stripe.Product, error) {
	...
	if err == nil {
		updated, err := stripeClient.V1Products.Update(ctx, product.ID, &stripe.ProductUpdateParams{
			Active: stripe.Bool(true),
			Name:   stripe.String(cfg.productDisplayName),
			Metadata: map[string]string{
				"forge_metal_product_id": cfg.productID,
			},
		})
		if err != nil {
	...
	created, err := stripeClient.V1Products.Create(ctx, &stripe.ProductCreateParams{
		ID:     stripe.String(stripeProductID),
		Active: stripe.Bool(true),
		Name:   stripe.String(cfg.productDisplayName),
		Metadata: map[string]string{
			"forge_metal_product_id": cfg.productID,
...
func upsertPlan(ctx context.Context, pg *sql.DB, cfg config, tier tierConfig, stripePriceID string) error {
	_, err := pg.ExecContext(ctx, `
		INSERT INTO plans (
			plan_id, product_id, display_name, billing_mode, quotas, is_default, tier, active,
			stripe_price_id_monthly, monthly_amount_cents, currency
		)
		VALUES ($1, $2, $3, 'prepaid', '{}'::jsonb, false, $4, true, $5, $6, $7)
...
func upsertPlanRates(ctx context.Context, pg *sql.DB, sourcePlanID string, planID string) error {
	result, err := pg.ExecContext(ctx, `
		INSERT INTO plan_sku_rates (plan_id, sku_id, unit_rate, active)
		SELECT $2, sku_id, unit_rate, active
		FROM plan_sku_rates
		WHERE plan_id = $1
	...
	if rows == 0 {
		return fmt.Errorf("source plan %s has no sku rates to copy", sourcePlanID)
	}
	return nil
}
	...
	for _, bucketID := range sortedEntitlementBuckets(tier.Entitlements) {
		...
		_, err := pg.ExecContext(ctx, `
			...
			SET source = EXCLUDED.source,
			    ...
			    updated_at = now()
		`, policyID, productID, bucketID, tier.Entitlements[bucketID])
		if err != nil {
...
func recordCatalogEvent(ctx context.Context, pg *sql.DB, reconciliationID string, cfg config, tier tierConfig, stripeProductID string, stripePriceID string) error {
...
func verifyPlan(ctx context.Context, pg *sql.DB, productID string, tier tierConfig, stripePriceID string) error {
