...
func (c *Client) ListContracts(ctx context.Context, orgID OrgID) ([]ContractRecord, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&row.ContractID, &row.ProductID, &row.PlanID, &row.PhaseID, &row.State, &row.PaymentState, &row.EntitlementState, &row.StartsAt, &row.EndsAt, &row.PhaseStart, &row.PhaseEnd); err != nil {
