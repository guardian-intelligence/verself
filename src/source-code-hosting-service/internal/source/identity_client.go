...
import (
	...
	identityclient "github.com/verself/identity-service/internalclient"
...
type IdentityClient struct {
	Client *identityclient.ClientWithResponses
}

func NewIdentityClient(baseURL string, httpClient identityclient.HttpRequestDoer) (IdentityClient, error) {
	client, err := identityclient.NewClientWithResponses(strings.TrimRight(baseURL, "/"), identityclient.WithHTTPClient(httpClient))
	if err != nil {
...
func (c IdentityClient) ResolveSourceOrganization(ctx context.Context, slug string) (_ OrganizationReference, err error) {
	return c.resolve(ctx, 0, slug)
}

func (c IdentityClient) ResolveSourceOrganizationID(ctx context.Context, orgID uint64) (_ OrganizationReference, err error) {
	return c.resolve(ctx, orgID, "")
}

func (c IdentityClient) resolve(ctx context.Context, orgID uint64, slug string) (_ OrganizationReference, err error) {
