...
var queryDocs = []queryDoc{
	...
	},
	{
		...
		Purpose: "Every Smithy OpenAPI projection, SDK generator, and frontend package codegen Bazel spawn that ran during one deploy. cache_hit=1 means the action graph was warm; cache_hit=0 means a contract or projection change forced a real codegen run.",
		Required: []string{
			"--run-key=<deploy-run-key>",
		},
		Optional: []string{
...
func printIndex(cfg config) error {
	...
	case formatMarkdown:
		...
		fmt.Println("\n## Time Windows")
		for _, item := range index.TimeWindows {
			fmt.Printf("- %s\n", item)
		}
