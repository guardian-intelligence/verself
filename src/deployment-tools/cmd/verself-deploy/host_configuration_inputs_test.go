...
func TestHostConfigurationSkipTags(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			changedPaths: []string{"src/vm-guest-telemetry/src/guest.zig"},
			want:         nil,
		},
		{
			name:         "vm bridge keeps guest image and firecracker work",
			changedPaths: []string{"src/vm-orchestrator/cmd/vm-bridge/main.go"},
			want:         nil,
		},
		{
			name:         "vm orchestrator daemon skips guest image build but keeps firecracker work",
			changedPaths: []string{"src/vm-orchestrator/server.go"},
