...
async function ensureTestUserExists(): Promise<void> {
  ...
  const projResp = await fetch(
    `${env.zitadelBaseURL}/management/v1/projects/_search`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({
  ...
  await fetch(
    `${env.zitadelBaseURL}/management/v1/users/${userId}/grants`,
    {
      method: "POST",
      headers,
...
async function completeStripeCheckout(page: Page): Promise<void> {
  ...
  const hasLinkOTP =
    (await page.locator("text=000000").count().catch(() => 0)) > 0;
...
test.describe("Credit Purchase Flow", () => {
  ...
  test("purchase $10 credit pack via Stripe, verify balance increase and billing history", async ({
    ...
    await expect(
      page.getByRole("heading", { name: "Purchase Credits" }),
    ).toBeVisible();
    ...
    await expect(
      page.getByText("Credits purchased successfully"),
    ...
    await expect(
      page.getByRole("heading", { name: "Active Credit Grants" }),
    ).toBeVisible();
  ...
  test("balance card shows purchased vs free tier breakdown", async ({
    page,
  }) => {
