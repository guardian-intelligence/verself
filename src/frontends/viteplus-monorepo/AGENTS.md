...
# repo root again
cd ../..
bazelisk run //src/frontends/viteplus-monorepo/apps/company:dev_update
bazelisk run //src/frontends/viteplus-monorepo/apps/verself-web:dev_update
```
...
`src/frontends/viteplus-monorepo/.npmrc` to:
...
**Shared frontend time abstraction:** use `formatUTCDateTime()` from `src/frontends/viteplus-monorepo/packages/web-env/src/time.ts` for SSR-visible timestamps in the web apps. It centralizes `Intl.DateTimeFormat` with `timeZone: "UTC"` and caches formatters so the rendered text agrees on the server and client.

### UI primitives

Use the shadcn/ui components from `src/frontends/viteplus-monorepo/packages/ui/src/components/ui`. They are shadcn-shaped exports wrapping Base UI (`@base-ui/react`) primitives, not Radix. Commit 4d7567b cut the whole stack over; Radix, cmdk, and vaul are gone.
