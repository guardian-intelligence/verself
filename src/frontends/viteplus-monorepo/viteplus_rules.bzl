...
_INSTRUMENTATION_BUNDLER = "//src/frontends/viteplus-monorepo/scripts:bundle_instrumentation"
...
def viteplus_app(npm_name, srcs, name = "instrumentation_bundle"):
    ...
    js_run_binary(
        name = name,
        srcs = [
            "//src/frontends/viteplus-monorepo:workspace_config",
            ":node_modules",
            ":sources",
            "instrumentation.mts",
        ],
...
def viteplus_route_tree(name):
    ...
    js_run_binary(
        name = gen_target,
        srcs = [
            "//src/frontends/viteplus-monorepo:node_modules/@tanstack/router-generator",
            ":sources",
        ],
        outs = [out],
        args = [
        ...
        chdir = native.package_name(),
        tool = "//src/frontends/viteplus-monorepo/scripts:generate_route_tree",
...
def viteplus_openapi_clients(name, specs, openapi_ts_bin, plugin_packages = None):
    ...
    for sdk_dir, spec_label in specs:
        ...
        js_run_binary(
            ...
            tool = "//src/frontends/viteplus-monorepo/scripts:openapi_postprocess",
