...
// rules_go's go_binary targets, packed into dev_tools.tar.zst alongside
// the pinned_http_file binaries. Each entry references a Bazel label
// auto-generated by gazelle's go_deps extension from the upstream Go
// module pinned in src/devtools/go.mod (or a verself service module
// for the protoc-* tools which were already vendored). The catalog pin
// is the version-of-record; the binary's actual version is enforced by
// go.sum, not by an http_file sha256.
//
// Adding a new Go-installable tool:
//   1. Add `require X v...` and `tool X/cmd/T` to src/devtools/go.mod
//   2. Run `go mod tidy` in src/devtools/ and `bazelisk mod tidy`
//   3. Verify the auto-generated label resolves via `bazelisk query`
//   4. Add an entry here
//   5. Flip the corresponding devTools entry to tier=source_built_go
sourceBuiltGoTools: {
	"golangci-lint": {
		bazel_label:  "@com_github_golangci_golangci_lint_v2//cmd/golangci-lint:golangci-lint"
		install_path: "/usr/local/bin/golangci-lint"
		version:      versions.development.golangciLint
	}
	gosec: {
		bazel_label:  "@com_github_securego_gosec_v2//cmd/gosec:gosec"
		install_path: "/usr/local/bin/gosec"
		version:      versions.development.gosec
	}
	gofumpt: {
		bazel_label:  "@cc_mvdan_gofumpt//:gofumpt"
		install_path: "/usr/local/bin/gofumpt"
		version:      versions.development.gofumpt
	}
	"protoc-gen-go": {
		bazel_label:  "@org_golang_google_protobuf//cmd/protoc-gen-go:protoc-gen-go"
		install_path: "/usr/local/bin/protoc-gen-go"
...
devTools: {
	...
	"golangci-lint": {
		...
		version: versions.development.golangciLint
		// Source-built Go binaries don't have upstream's release-time
		// -ldflags version stamp, so the runtime version string is
		// unreliable. The binary's bytes are pinned via go.sum at build
		// time; the version_check span just confirms the binary is on
		// PATH and exits zero. Drift detection lives in the build-time
		// `go list -m` gate, not in deploy-time runtime introspection.
		version_cmd: "golangci-lint --version"
	}
	gosec: {
		tier:        #DevToolTier & "source_built_go"
		version:     versions.development.gosec
		version_cmd: "gosec --help"
	}
	gofumpt: {
		tier:        #DevToolTier & "source_built_go"
		version:     versions.development.gofumpt
