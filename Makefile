.PHONY: help bazel-doctor bazel-smoke-test bazel-gazelle bazel-tidy bazel-update test lint lint-scripts lint-conversions lint-ansible lint-voice company-smoke-test fmt vet tidy sqlc sqlc-check openapi openapi-check openapi-clients openapi-clients-check openapi-wire-check topology-generate topology-check \
       ...
       wipe-pg-db wipe-server vm-orchestrator-smoke-test firecracker-cutover-static-test sandbox-inner sandbox-middle sandbox-smoke-test console-ui-smoke console-ui-local console-local-dev console-frontend-deploy-fast grafana-smoke-test observability-smoke services-doctor
...
firecracker-cutover-static-test: ## Static source-tree asserts for the substrate + toolchain + bridge cleanup (no prod dep)
	$(PLATFORM_DIR)/scripts/verify-firecracker-cutover-static.sh
