...
High-level topology — components, ports, SPIRE identities, runtime users — is authored in `src/host-configuration/ansible/group_vars/all/topology/` and service-owned Nomad metadata under each deployable package. Host firewall files are authored in `src/host-configuration/ansible/host-files/`. HAProxy public edge templates are authored in `src/host-configuration/ansible/roles/haproxy/templates/`. Bazel-input artefacts are authored in their owner packages: `src/host-configuration/binaries/`, `src/tools/dev/binaries/`, and `src/substrate/vm-orchestrator/guest-images/`. Run `aspect operator edge --action=check` to validate route topology, Nomad service registrations, and HAProxy object identity.
