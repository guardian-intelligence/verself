...
High-level topology — components, ports, SPIRE identities, runtime users — is declared in `src/cue-renderer/` and materialised by `aspect render --site=<site>` into `.cache/render/<site>/`: typed Ansible inputs under `inventory/group_vars/all/generated/*.yml`, and final-file copies under `share/rendered/` for the host firewall (`etc/nftables.conf`, `etc/nftables.d/<component>.nft`, `etc/systemd/system/verself-firewall.target`). The cache is gitignored; the source of truth is CUE. Bazel-input artefacts (`src/cue-renderer/binaries/{server,dev}_tools.{MODULE.bazel,bzl}`, `src/vm-orchestrator/guest-images/guest_images.MODULE.bazel`) remain tracked via `write_source_files` because Bazel evaluates them at load time; refresh them with `bazelisk run //src/cue-renderer:dev_update`. Run `python3 src/substrate/scripts/services-doctor.py` to cross-check generated topology endpoints against live listeners on the box (supports `--format=json|nftables`).
