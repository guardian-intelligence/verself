...
High-level topology — components, ports, SPIRE identities, runtime users — is declared in `src/cue-renderer/` and materialised by `aspect render --site=<site>` into `.cache/render/<site>/`: typed Ansible inputs under `inventory/group_vars/all/generated/*.yml`, and final-file copies under `share/rendered/` for the host firewall (`etc/nftables.conf`, `etc/nftables.d/<component>.nft`, `etc/systemd/system/verself-firewall.target`). The cache is gitignored; the source of truth for deploy topology is CUE. Bazel-input artefacts are authored in their owner packages: `src/substrate/binaries/`, `src/devtools/binaries/`, and `src/vm-orchestrator/guest-images/`. Run `python3 src/substrate/scripts/services-doctor.py` to cross-check generated topology endpoints against live listeners on the box (supports `--format=json|nftables`).
