...
- Verself actions such as `verself/cache`, `guardian-intelligence/verself-checkpoint`, and
...
| Verself Checkpoints | A keyed ext4 filesystem generation is restored from a ZFS snapshot, mounted at a requested path, and committed after successful saveback. | `guardian-intelligence/verself-checkpoint`, API | Rewrite target |
...
The harness uses `guardian-intelligence/verself-checkpoint@v0` in the workload workflow and keeps
...
2. `guardian-intelligence/verself-checkpoint@v0` runs as a normal GitHub Action step. The request
