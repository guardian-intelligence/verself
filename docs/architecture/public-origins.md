...
`src/host-configuration/edgecontract` compiles authored route topology, endpoint topology, authored Nomad jobs, and HAProxy defaults into the public edge plan. `aspect operator edge --action=check` validates that topology routes, Nomad service registrations, upstream keys, and HAProxy GUID-bearing object identities remain consistent. `aspect operator edge --action=manifest --format=json` emits route backends, Nomad upstream keys, and HAProxy object identities for review and telemetry correlation.
