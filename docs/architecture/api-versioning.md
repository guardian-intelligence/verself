...
`apiversion-changelog` CLI emits markdown for `src/websites/apps/verself-web/src/routes/_workshop/policy/changelog.tsx`. `Hidden()` changes are excluded from rendered output but retained in the structured form.
